...
sub Run {
	...
	$Kernel::OM->Get('Kernel::System::Log')->Log(Priority => "error" , Message => "VALORES", );
    ...
    if ( $CategoryIDsRef && ref $CategoryIDsRef eq 'ARRAY' && @{$CategoryIDsRef} ) {
        ...
        for my $SubCategoryID ( @{$CategoryIDsRef} ) {
			my $SubCategoryIDArrayRef = $FAQObject->CategorySubCategoryIDList(
     		...
     		my $values = $SubCategoryID; 
			
			for ( @{$SubCategoryIDArrayRef}){
				$values .= "," . $_;
			}
		 		$Kernel::OM->Get("Kernel::System::Log")->Log(Priority => "error" , Message => "VALORES". $values);
			#COMPLEMENTO
